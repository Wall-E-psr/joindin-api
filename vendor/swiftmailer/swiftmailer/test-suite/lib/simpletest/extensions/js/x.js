...
// Size:
    ...
    function xClip(e,iTop,iRight,iBottom,iLeft)
    {
        ...
        if (e.style) {
            ...
            if (xNum(iLeft)) {
                e.clip.top=iTop; e.clip.right=iRight; e.clip.bottom=iBottom; e.clip.left=iLeft; } else {
                e.clip.top=0; e.clip.right=xWidth(e); e.clip.bottom=xHeight(e); e.clip.left=0; }
        }
    }
// Window:
...
// Animation:
    ...
    function xSlide(e)
    {
        ...
        t = now.getTime() - e.C;
        if (e.stop) {
            e.moving = false; } else if (t < e.slideTime) {
...
// Event:
    function xAddEventListener(e,eventType,eventListener,useCapture)
    {
        ...
        if ((!xIE4Up && !xOp7) && e==window) {
            if (eventType=='resize') {
                window.xPCW=xClientWidth(); window.xPCH=xClientHeight(); window.xREL=eventListener; xResizeEvent(); return; }
            if (eventType=='scroll') {
                window.xPSL=xScrollLeft(); window.xPST=xScrollTop(); window.xSEL=eventListener; xScrollEvent(); return; }
        }
    ...
    function xEvent(evt)
    {
     // cross-browser event object prototype
        ...
        if (xOp5or6) {
            this.pageX = e.clientX; this.pageY = e.clientY; } else if (xDef(e.pageX,e.pageY)) {
            this.pageX = e.pageX; this.pageY = e.pageY; } // v3.14
            else if (xDef(e.clientX,e.clientY)) {
                this.pageX = e.clientX + xScrollLeft(); this.pageY = e.clientY + xScrollTop(); }
            if (xDef(e.offsetX,e.offsetY)) {
                this.offsetX = e.offsetX; this.offsetY = e.offsetY; } else if (xDef(e.layerX,e.layerY)) {
                this.offsetX = e.layerX; this.offsetY = e.layerY; } else {
                    this.offsetX = this.pageX - xPageX(this.target); this.offsetY = this.pageY - xPageY(this.target); }
                if (e.keyCode) {
                    this.keyCode = e.keyCode; } // for moz/fb, if keyCode==0 use which
                else if (xDef(e.which)) {
                    this.keyCode = e.which; }
    }
    function xResizeEvent()
    {
     // window resize event simulation
        if (window.xREL) {
            setTimeout('xResizeEvent()', 250);
        }
        var cw = xClientWidth(), ch = xClientHeight();
        if (window.xPCW != cw || window.xPCH != ch) {
            window.xPCW = cw; window.xPCH = ch; if (window.xREL) {
                window.xREL();
            } }
    }
    function xScrollEvent()
    {
     // window scroll event simulation
        if (window.xSEL) {
            setTimeout('xScrollEvent()', 250);
        }
        var sl = xScrollLeft(), st = xScrollTop();
        if (window.xPSL != sl || window.xPST != st) {
            window.xPSL = sl; window.xPST = st; if (window.xSEL) {
                window.xSEL();
            } }
    }
// end x.js
